...
function mkconf(conf, cb=null) {
  ...
  conf = Object.assign({
    depth: -1, self: false, first: false,
    id: null, tag: null,
    leaf: false, stem: false, root: false,
    cb,
  }, conf)
  if (typeof(conf.tag) == 'string') conf.tag = conf.tag.split(' ')
  conf.glob = !conf.id && !conf.tag && !conf.root && !conf.stem && !conf.leaf
  return conf
}
...
class AST {
  ...
  get sequence() {
    return this.each({self: true})
  }

  // Return a flattened array of only the tree's leaves
  get leafSequence() {
    return this.each({self: true, leaf: true})
  }
  ...
  */
  ...
  ancestor(conf, cb=null) {
    ...
    if (this.parent.match(conf)) {
      if (conf.cb) conf.cb(this.parent)
      return this.parent
    } else return this.parent.ancestor(conf)
  }

  match(conf) {
    return conf.glob ||
      conf.id && this.id == conf.id ||
  ...
  each(conf={}, cb) {
    ...
    if (!conf.first || !matched.length) {
      for (let i = 0; i < this.nodes.length; i++) {
        ...
        if (conf.first && matched.length) break
        if (conf.depth < 0 || conf.depth > 0) {
          const submatch = node.each({...conf, cb: null, depth: conf.depth - 1})
          if (submatch?.length) {
    ...
    else if (conf.first) return matched[0]
    else return matched
  }
